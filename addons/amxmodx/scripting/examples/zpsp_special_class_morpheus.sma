...
// Variables
new g_gameid, g_msg_sync, cvar_minplayers, g_speciald, cvar_damage, acess_flags[2]
...
#define GetUserMorpheus(%1) (zp_get_human_special_class(%1) == g_speciald)

public plugin_init()
{
	// Plugin registeration.
	register_plugin("[ZP] Class Morpheus","1.2", "[P]erfec[T] [S]cr[@]s[H]")
...
public plugin_precache() {
	...
	if(!amx_load_setting_string(ZP_CUSTOMIZATION_FILE, "Access Flags", "START MODE MORPHEUS", user_access, charsmax(user_access))) {
		amx_save_setting_string(ZP_CUSTOMIZATION_FILE, "Access Flags", "START MODE MORPHEUS", default_flag_access)
	...
	if(!amx_load_setting_string(ZP_CUSTOMIZATION_FILE, "Access Flags", "MAKE MORPHEUS", user_access, charsmax(user_access))) {
		amx_save_setting_string(ZP_CUSTOMIZATION_FILE, "Access Flags", "MAKE MORPHEUS", default_flag_access)
		formatex(user_access, charsmax(user_access), default_flag_access)
	}
	...
	for (i = 0; i < ArraySize(g_sound_morpheus); i++) {
		ArrayGetString(g_sound_morpheus, i, buffer, charsmax(buffer))
...
public fw_TakeDamage(victim, inflictor, attacker, Float:damage, damage_type) {
	if(is_user_alive(attacker) && GetUserMorpheus(attacker) && get_user_weapon(attacker) == CSW_MP5NAVY)
